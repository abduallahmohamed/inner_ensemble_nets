...
def main_worker(gpu, ngpus_per_node, args):
    ...
    #print("Total Parameters in human readable format:",human_format(params))
    ...
    if args.prune:
        if args.variation == 'base':
                ...
                for id,module in enumerate(model.modules()):
                    if(type(module) == nn.Conv2d or type(module) == nn.Linear ):
                        if id in weights_dict:
                            ...
                            if module.bias is not None:
                                bias_dict[id].append(module.bias.data.clone())
            #Fuse the weights and bias dicts into a single model
            for id,module in enumerate(model.modules()):
                if(type(module) == nn.Conv2d or type(module) == nn.Linear ):
            ...
            pruned_acc = validate(val_loader, model, criterion, args)
            ...
            if args.gpu is None:
                ...
                bestpoint = torch.load(bestpath, map_location=loc)
            model.load_state_dict(bestpoint['state_dict'])
            pre_prune_best_acc = validate(val_loader, model, criterion, args)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0:
            progress.display(i)
    print("EpochTime: {}".format(batch_time.sum))
