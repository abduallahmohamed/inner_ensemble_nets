...
parser.add_argument('--prune', action='store_true',
                     help='prune and validate the best model')
parser.add_argument('-v', '--version', default=None, type=int,
                     help='version number of experiment run')
...
def main():
    ...
    #Save args
    with open(os.path.join(args.save_dir,'{}_args_ver_{}_m_{}.pkl'.format(args.arch, args.version, args.Mense)), 'wb') as f:
        pickle.dump(args, f)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.prune:
        ...
        for version in [0, 1, 2]:
            bestpath = os.path.join(args.save_dir, '{}_model_best_ver_{}_m_{}.pth.tar'.format(args.arch, version, args.Mense))
            bestpaths.append(bestpath)
        #bestpath = args.arch+'_model_best.pth.tar'
            print("=> loading bestpoint '{}'".format(bestpath))
            if args.gpu is None:
                bestpoint = torch.load(bestpath)
            else:
                # Map model to be loaded to specified single gpu.
                loc = 'cuda:{}'.format(args.gpu)
                bestpoint = torch.load(bestpath, map_location=loc)
            model.load_state_dict(bestpoint['state_dict'])
            model_copy = copy.deepcopy(model)
            models_ens.append(model_copy)
        pre_prune_best_acc = validate(val_loader, models_ens, criterion, args)
        print("Best accuracy before pruning: {}".format(pre_prune_best_acc))
        cnt_=0
        #Prune
        for model in models_ens:
            for m in model.modules():
                if hasattr(m, "domms"):
                    ...
                    cnt_+=1
                    print("CNT:",cnt_)
        pruned_acc = validate(val_loader, models_ens, criterion, args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best, filename=os.path.join(args.save_dir, '{}_checkpoint_ver_{}_m_{}.pth.tar'.format(args.arch, args.version, args.Mense)),
            bestname=os.path.join(args.save_dir, '{}_model_best_ver_{}_m_{}.pth.tar'.format(args.arch, args.version, args.Mense)))
            #Keep track of number of epochs completed in external text file
            f = open(os.path.join(args.save_dir,'{}_epochs_ver_{}_m_{}.txt'.format(args.arch, args.version, args.Mense)), 'w')
